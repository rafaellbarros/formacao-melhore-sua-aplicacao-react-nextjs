...
export const Box = React.forwardRef(({ as, styleSheet: { focus, hover, srOnly, ...styleSheet }, ...props }, ref) => {
  const Tag = as || 'div'
  ...
  return (
    <React.Fragment>
      <Box styleSheet={styleSheet}>
        <FontAwesomeIcon icon={iconSet[`fa${capitalize(iconVariant)}`]} crossOrigin="anonymous" {...props} />
      </Box>
    </React.Fragment>
  )
}
